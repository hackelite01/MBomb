...
```Due the amount of requests, the APIs can die. To prevent a bigger outtake of MBomb, it has been limited.``` 
...
- Help, i can't execute MBomb.sh!

Run MBomb Directly with
```python3 bomber.py```

- VPN? Proxy's? 

```No, MBomb can fail due the high response time or API restrictions.```
